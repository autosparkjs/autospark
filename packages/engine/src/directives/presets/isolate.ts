...
/**
 * x-isolate：engine 边界 / 隔离快照 / 远程子引擎（ADR-0006）。
 *
 * 在模板中划一块**独立于 engine 的隔离 DOM 区域**。两种模式（由值有无切换，二选一）：
 *
 * - **static**（无值 `<div x-isolate>`）：宿主内为**冻结快照**——`compile()` 深克隆 template 子节点、
 *   剥除全部指令属性（x- 前缀及 `@` / `:` 快捷前缀）、**不编译、不建 scope、不注册 watcher**。engine 永不覆写，
 *   开发者用 DOM API 全权管理。内层指令/`{{}}` 一律静默失效（编译期 warn 对冲）。
 *
 * - **remote**（`<div x-isolate="expr">`）：expr 经 `scope.watch` 求值得 **url（响应式，支持路径/
...
export class IsolateDirective extends AutoSparkDirectiveBase {
    ...
    /**
     * x-isolate 永远占有子树：static 自行克隆填充、remote 由 child engine 接管子节点，
    ...
    private async _loadUrl(url: any): Promise<void> {
        ...
        try {
            ...
            // （isolate → engine → manager → presets → isolate），且子类化 AutoSpark 时自动跟随。
    ...
    private _warnIfInnerDirectives(root: HTMLElement): void {
        ...
        if (found) {
            this.warn(
                `x-isolate: 静态内容不编译，内部指令/{{}} 不生效；若需响应式请用普通元素或 x-isolate="url" 远程子引擎`,
            );
        }
    }

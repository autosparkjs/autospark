...
<div id="b"><span x-text="x.y.v"></span></div>`,
        ...
        expect(engine.state.x.y.v).toBe(1);
    ...
    test("中间路径不存在时逐级自动创建：mount:'x.y' 建出 x:{y:{...}}", () => {
        const { engine } = mount(`<div x-data="{a:1}" x-data-options="{mount:'p.q.r'}"></div>`, {});
        expect(engine.state.p.q.r).toEqual({ a: 1 });
    });

    test("merge 语义：挂载点已有旧键保留，新键追加", () => {
        const { engine } = mount(`<div x-data="{a:1}" x-data-options="{mount:'box'}"></div>`, {
            box: { keep: "旧值" },
        });
        expect(engine.state.box).toEqual({ keep: "旧值", a: 1 });
    });
    ...
    test("path 模式 this.data 可写 + engine.data 直 merge 进挂载容器（细粒度响应式）", async () => {
        ...
        // this.data 写：改 engine.state.cfg.n 即触发订阅
        engine.state.cfg.n = 5;
        await nextTick();
        ...
        expect(engine.state.cfg.m).toBe(9);
        expect(engine.state[SCOPES_KEY]).not.toHaveProperty(
...
<span id="out" x-text="g.v"></span>`,
    ...
    test("中途断裂（存在但非对象）：warn + 降级默认私有域，数据不丢", () => {
        ...
        expect((engine.state as any).x).toBe(5);
        expect(root.querySelector("#a span")!.textContent).toBe("1");
        const id = engine.findScopeByEl(root.querySelector("#a")!)!.id;
        expect(engine.state[SCOPES_KEY][id]).toEqual({ a: 1 });
    });
    ...
    test("mount 非字符串（误写 .mount 修饰符产生 true）：warn + 忽略，回默认私有域", () => {
        ...
        expect(root.querySelector("#a span")!.textContent).toBe("1");
        expect((engine.state as any).a).toBeUndefined();
    });

    test("mount 空串 '' 等价根（≡ .global）", () => {
        const { engine } = mount(`<div x-data="{k:1}" x-data-options="{mount:''}"></div>`, {});
        expect(engine.state.k).toBe(1);
    });
    ...
    test("mount 与 global 同写：mount 优先 + 不写根键", () => {
        ...
        expect(engine.state.m.k).toBe(1);
        expect((engine.state as any).k).toBeUndefined();
    });

    test("global 带路径残留写法：warn + 按根处理", () => {
        const { engine } = mount(`<div x-data="{k:1}" x-data-options="{global:'x.y'}"></div>`, {});
        expect(engine.state.k).toBe(1);
    });
});

describe("x-data mount 相对路径挂载", () => {
    test("'./x' 挂到自身容器下：$scopes.<id>.x", () => {
        const { root, engine } = mount(
            `<div id="a" x-data="{v:1}" x-data-options="{mount:'./x'}"><span x-text="v"></span></div>`,
            {},
        );
        const id = engine.findScopeByEl(root.querySelector("#a")!)!.id;
        expect(engine.state[SCOPES_KEY][id]).toEqual({ x: { v: 1 } });
        expect(root.querySelector("#a span")!.textContent).toBe("1");
    });

    test("'../shared' 挂到直接父 scope（父有 x-data）：$scopes.<pid>.shared", () => {
        const { root, engine } = mount(
        ...
        expect(engine.state[SCOPES_KEY][pid]).toEqual({ pv: 0, shared: { v: 1 } });
    });
    ...
    test("'../x' 父 scope 无 _data：无容器则创建（空私有域 + 数据挂入）", () => {
        ...
        expect(engine.state[SCOPES_KEY][mid.id]).toEqual({ up: { v: 1 } });
    });
    ...
    test("'../x' 越顶（根 scope 无父）：落根 state", () => {
        ...
        expect((engine.state as any).top).toEqual({ v: 1 });
    });
    ...
    test("'../../x' 走两级直接父 scope", () => {
        ...
        expect(engine.state[SCOPES_KEY][gid]).toEqual({ g: 0, deep: { v: 1 } });
    });
    ...
    test("相对路径多段后缀：'../settings/theme' 挂到父容器下的 settings.theme", () => {
        ...
        const pid = engine.findScopeByEl(root.querySelector("#p")!)!.id;
        expect(engine.state[SCOPES_KEY][pid].settings.theme).toEqual({ v: 1 });
    });
});
...
describe("x-data mount .nearest 修饰符", () => {
    test("nearest：.. 跳过无 _data 的占位 scope，挂到最近持有数据的祖先", () => {
        ...
        expect(engine.state[SCOPES_KEY][gid]).toEqual({ g: 0, near: { v: 1 } });
    });
    ...
    test('nearest 经 x-data-options="{nearest:true}" 等价', () => {
        ...
        const gid = engine.findScopeByEl(root.querySelector("#gp")!)!.id;
        expect(engine.state[SCOPES_KEY][gid].near).toEqual({ v: 1 });
    });
    ...
    test("nearest 上溯无数据祖先：落根 state", () => {
        ...
        expect((engine.state as any).root).toEqual({ v: 1 });
    });

    test("nearest 命中 path 模式祖先：挂到其挂载容器下（非 $scopes）", () => {
        ...
        expect(engine.state.base).toEqual({ g: 0, child: { v: 1 } });
    });
    ...
    test("nearest 配绝对路径：静默忽略（正常挂载）", () => {
        ...
        expect(engine.state.abs).toEqual({ v: 1 });
    });
    ...
    test("默认（无 nearest）.. 不跳层：直接父是占位 scope 时为其创建容器（对照）", () => {
        ...
        expect(engine.state[SCOPES_KEY][mid.id]).toEqual({ near: { v: 1 } });
        expect(engine.state[SCOPES_KEY][gid]).toEqual({ g: 0 });
    });
});
...
describe("x-data mount destroy 回收", () => {
    test("path 模式 destroy：键级 CAS 删除 + 容器删空则连同中间容器向上回收", () => {
        ...
        expect(engine.state.p.q.r).toEqual({ v: 1 });
        destroyScopeFor(engine, root.querySelector("#a")!);
        // p.q.r 删空 → 连 q、p 一并回收
        expect((engine.state as any).p).toBeUndefined();
    });
    ...
    test("挂载点他人旧键保留：删自己的键后容器非空 → 不回收容器", () => {
        ...
        destroyScopeFor(engine, root.querySelector("#a")!);
        expect(engine.state.box).toEqual({ keep: "旧值" });
    });
    ...
    test("engine.data 运行时追加的键不回收（残留可接受，用户接管）", async () => {
        ...
        expect(engine.state.box).toEqual({ runtimeKey: 1 });
    });
...
<div id="e2" x-data="{x:2}" x-data-options="{mount:'shared'}"></div>`,
        ...
        expect(engine.state.shared.x).toBe(2);
        destroyScopeFor(engine, root.querySelector("#e1")!);
        expect(engine.state.shared.x).toBe(2); // e1 的 1 已被覆盖，CAS 跳过
        destroyScopeFor(engine, root.querySelector("#e2")!);
        expect((engine.state as any).shared).toBeUndefined(); // e2 是末写者，删空后容器回收
    });
    ...
    test("无效路径降级 local 后 destroy：按私有域回收（不留垃圾）", () => {
        ...
        expect(engine.state[SCOPES_KEY][id]).toEqual({ a: 1 });
        destroyScopeFor(engine, root.querySelector("#a")!);
        expect((engine.state as any)[SCOPES_KEY][id]).toBeUndefined();
        expect((engine.state as any).x).toBe(5); // 用户数据不动
    });

    test("engine.destroy 全量销毁：mount 挂载的容器一并回收", () => {
        const { engine } = mount(`<div x-data="{v:1}" x-data-options="{mount:'p.q'}"></div>`, {});
        expect(engine.state.p.q).toEqual({ v: 1 });
        engine.destroy();
        expect((engine.state as any).p).toBeUndefined();
    });
});

describe("x-data mount 指向 $scopes 他域（warn + 放行）", () => {
    test("mount:'$scopes.<id>' 直指他域私有域：放行 merge", () => {
...
<div id="attacker" x-data="{a:2}" x-data-options="{mount:'$scopes.${nextId}'}"></div>`,
            {},
        );
        // host 恰取 nextId，attacker 的数据 merge 进其私有域（warn + 放行语义）
        expect(engine.state.$scopes[nextId]).toEqual({ h: 1, a: 2 });
    });
});

...
describe("x-for v2 key-based 节点复用", () => {
    ...
    test("unshift 加首项：旧项 index 变 → 重订阅（项根 DOM 身份保持，P1）", async () => {
        ...
        engine.state.items.unshift({ id: 0 });
        await nextTick();
        expect(ul.children.length).toBe(3);
    ...
    test("P0: items[i]={同id新对象} 整体替换单项 → 项内容更新（旧版静默失效已修复）", async () => {
        ...
        engine.state.items[0] = { id: 1, name: "REPLACED" };
        await nextTick();
    ...
    test("中间 splice 插入：后续项 index 变 → 重建，新项就位", async () => {
        ...
        engine.state.items.splice(1, 0, { id: 2 });
        await nextTick();
        expect(ul.children.length).toBe(3);
    ...
    test("整体替换（同 key 不同引用）：复用 + refresh，DOM 身份保持且内容更新", async () => {
        ...
        engine.state.items = [{ id: 1, name: "A2" }];
        await nextTick();
    ...
    test("复合项 push：组内各成员 DOM 身份保持", async () => {
        ...
        engine.state.items.push({ id: 2, k: "b", v: "2" });
        await nextTick();
    ...
    test("无 :key：push 旧项复用（index 不变），unshift 旧项重建（index 变）", async () => {
        ...
        engine.state.items.unshift({ name: "z" });
        await nextTick();
    ...
    test("filter 表达式：toggle active 增删差异项，存留项 DOM 身份保持", async () => {
        ...
        engine.state.items.push({ id: 3, active: true });
        await nextTick();
    ...
    test("嵌套 x-for：外层 push 复用外层 li，最终 HTML 正确", async () => {
        ...
        engine.state.matrix.push({ id: 3, title: "r3", cells: ["d", "e"] });
        await nextTick();
        // 外层前两项 index 不变 → 复用
  ...
  <ul>
    ...
    test("反复结构变化稳定：交替 push/splice/整体替换不抛错、最终态正确", async () => {
        ...
        const ul = root.querySelector("ul")!;
        engine.state.items.push({ id: 4 });
        await nextTick();
        engine.state.items.splice(1, 1); // 删 id=2
        await nextTick();
        engine.state.items = [{ id: 5 }, { id: 6 }];
        await nextTick();
        engine.state.items.unshift({ id: 0 });
        await nextTick();
        expect(ul.children.length).toBe(3);
  ...
  <ul>
    ...
    test("P1 复合项移动：组内各成员项根 DOM 均复用（dt/dd 各自保持）", async () => {
        ...
        engine.state.items.unshift({ id: 0, k: "z", v: "0" });
        await nextTick();
  ...
  <dl>
    ...
    test("P0 不误伤字段级：items[i].field= 走字段级精准 patch，DOM 不重建", async () => {
        ...
        const li0 = ul.children[0]!;
        const li1 = ul.children[1]!;
        engine.state.items[0].name = "A2"; // 原位改字段，不换对象
        await nextTick();

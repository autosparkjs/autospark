...
/**
 * core 深层动态键响应式契约——x-data 私有域（store.state.$scopes[id]）的命门假设。
 *
 * engine 运行时往 store.state.$scopes 注入容器、DataDirective 运行时为每个 x-data scope
 * 建 $scopes[id] 对象并向其 Object.assign / delete 键。这依赖 core 对「初始 state 不存在、
 ...
 */
describe("core 深层动态键响应式契约（$scopes 私有域命门）", () => {
    test("运行时建 $scopes[id] 并改其字段：订阅者收到通知", () => {
        const store = new AutoStore({} as any);
        // engine 行为：注入 $scopes 容器
        (store.state as any)[SCOPES_KEY] = {};
        // DataDirective 行为：运行时为某 scope 建 $scopes[id] 对象（core 须自动建响应式代理）
        (store.state as any)[SCOPES_KEY][12] = { count: 1 };

        // collectDependencies 应能收集到 $scopes.12.count 路径
        const deps = store.collectDependencies(() => {
            return (store.state as any)[SCOPES_KEY][12].count;
        });
        expect(deps.some((p) => p.join(".") === "$scopes.12.count")).toBe(true);
    ...
    test("运行时在已响应式的 $scopes[id] 上新增键：新键亦可订阅+通知", () => {
        const store = new AutoStore({} as any);
        (store.state as any)[SCOPES_KEY] = {};
        (store.state as any)[SCOPES_KEY][12] = { count: 1 };
        ...
        expect(deps.some((p) => p.join(".") === "$scopes.12.name")).toBe(true);
    ...
    test("delete $scopes[id] 的键：订阅者收到通知（DataDirective 删消失键）", () => {
        const store = new AutoStore({} as any);
        (store.state as any)[SCOPES_KEY] = {};
        (store.state as any)[SCOPES_KEY][12] = { count: 1 };

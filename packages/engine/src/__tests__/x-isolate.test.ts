...
describe("x-isolate 静态冻结快照", () => {
    test("static：渲染 template 子节点内容", () => {
        const { root } = mount(`<div x-isolate><a href="x">ssss</a></div>`, {});
  ...
  <div><a href="x">ssss</a></div>
    ...
    test("static：内容不响应状态变化（无 watcher，T1 防御）", async () => {
        const { root, engine } = mount(`<div x-isolate><span>fixed</span></div>`, { x: 1 });
    ...
    test("static：剥除内部指令属性（x-text 不绑定、属性消失、文本保持字面）", async () => {
        const { root, engine } = mount(`<div x-isolate><span x-text="t">old</span></div>`, {
            t: "NEW",
        });
    ...
    test("static：内部含指令时不抛错（warn 对冲，内容照常渲染）", () => {
        const { root } = mount(`<div x-isolate><b x-text="t">x</b></div>`, { t: "y" });
    ...
    test("static：DOM API 改动在反应式刷新后保留（engine 不触碰内容）", async () => {
        const { root, engine } = mount(
            `<div><main x-isolate><p>hi</p></main><span x-text="counter"></span></div>`,
            { counter: 0 },
        );

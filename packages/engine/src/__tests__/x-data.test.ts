...
describe("engine.data 运行时更新", () => {
  ...
  <div id="inner" x-data="{a:2}">
    <span x-text="a"></span>
            ...
            {},
        );         // 初始：内层 a=2 覆盖外层 a=1，依赖路径分别为 $scopes.<outer>.a / $scopes.<inner>.a
        expect(root).toEqualHTML(`<div>
  <div id="outer">
    <span>1</span>
    <div id="inner">
...
describe("x-data.global 挂载全局 store", () => {
  ...
  <div id="a">
    ...
    test("响应式：改 engine.state 自动更新（global 运行时改用 engine.state 直接操作）", async () => {
        const { root, engine } = mount(
  ...
  <div>
    ...
    test("销毁 CAS：元素销毁后自动删除自己写入的键", () => {
        ...
        expect(engine.state.a).toBe(1);
        expect(engine.state.b).toBe(2);
        engine.destroy();
        expect(engine.state.a).toBeUndefined();
        expect(engine.state.b).toBeUndefined();
    });
});
...
describe("x-data 配置等价性（ADR-0007：modifier ≡ 指令选项，宿主选项回退）", () => {
    test('x-data.global ≡ x-data-options="{global:true}"：均写入 store 根键', () => {
        ...
        expect(a.engine.state.eq).toBe(1);
        expect(b.engine.state.eq).toBe(2);
    });
    ...
    test("宿主选项回退：x-data 无 global 配置时回退读取 x-options.global", () => {
        ...
        const { engine } = mount(`<div x-options="{global:true}" x-data="{host:1}"></div>`, {});
        expect(engine.state.host).toBe(1);
    });
    ...
    test("指令选项优先于宿主选项：x-data-options.global=false 显式关闭", () => {
        ...
        expect(engine.state.x).toBeUndefined();
    });
});
...
describe("x-data.global 多实例碰撞（CAS 销毁）", () => {
    ...
    test("部分键重叠：CAS 保留后写者的值，先写者只清理自己的独占键", () => {
        ...
        // b 被 e2 覆盖为 9
        expect(engine.state.a).toBe(1);
        expect(engine.state.b).toBe(9);
        expect(engine.state.c).toBe(3);
        ...
        destroyScopeFor(engine, root.querySelector("#e2")!);
        expect(engine.state.b).toBeUndefined();
        expect(engine.state.c).toBeUndefined();
    });
    ...
    test("engine.destroy 全量销毁：多实例碰撞下最终 store 干净（末写者删除、其余 CAS 跳过）", () => {
        ...
        expect(engine.state.a).toBe(2);
        expect(engine.state.b).toBe(3);
        engine.destroy();

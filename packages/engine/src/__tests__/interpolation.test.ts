...
describe("文本插值 {{ }}", () => {
    test("变量：初始渲染 + 状态变化更新 DOM", async () => {
        const { root, engine } = mount(`<div>I am {{name}}</div>`, { name: "zhang" });
        expect(root).toEqualHTML(`<div>
  <div>I am zhang</div>
</div>`);
        engine.state.name = "li";
        await nextTick();
        expect(root).toEqualHTML(`<div>
  <div>I am li</div>
</div>`);
    ...
    test("表达式：多依赖自动收集，任一变化即重算", async () => {
        const { root, engine } = mount(`<div>{{ first + " " + last }}</div>`, {
            first: "zhang",
            last: "san",
        });
        expect(root).toEqualHTML(`<div>
  <div>zhang san</div>
        ...
        engine.state.first = "li";
        await nextTick();
        expect(root).toEqualHTML(`<div>
  <div>li san</div>
</div>`);
        engine.state.last = "si";
        await nextTick();
        expect(root).toEqualHTML(`<div>
  <div>li si</div>
</div>`);
    ...
    test("多段：同一文本节点多个 {{}} 各自独立响应", async () => {
        const { root, engine } = mount(`<p>I am {{a}} and {{b}}</p>`, { a: "1", b: "2" });
        expect(root).toEqualHTML(`<div>
  <p>I am 1 and 2</p>
</div>`);
        engine.state.b = "3";
  ...
  <p>I am 1 and 3</p>
    ...
    test("转义：插值结果作为纯文本，不注入 HTML（XSS 安全）", async () => {
        const { root, engine } = mount(`<div>{{html}}</div>`, { html: "<b>x</b>" });
        const div = root.querySelector("div")!;
        expect(div.children.length).toBe(0);
        expect(div.textContent).toBe("<b>x</b>");
        engine.state.html = "<img src=x onerror=alert(1)>";
        await nextTick();
        expect(div.children.length).toBe(0);
    ...
    test("directive-less 元素自动建 scope（隐式指令）：嵌套静态子树亦生效", async () => {
        const { root, engine } = mount(`<section><span>Hi {{name}}</span></section>`, {
            name: "k",
        });
        expect(root).toEqualHTML(`<div>
  <section>
  ...
  <section>
    ...
    test("x-text 在场：插值文本剪枝，x-text 独占 textContent（无孤儿 watcher）", async () => {
        const { root, engine } = mount(`<div x-text="name">{{age}}</div>`, {
            name: "zhang",
            age: 18,
        });
        expect(root).toEqualHTML(`<div>
...
describe("x-for 项内文本插值", () => {
  ...
  <ul>
    ...
    test("primitive 循环变量：数组变更触发重建渲染", async () => {
        const { root, engine } = mount(`<ul x-for="n of nums"><li>{{n}}</li></ul>`, {
            nums: [1, 2, 3],
        });
        expect(root).toEqualHTML(`<div>
  <ul>
...
describe("属性插值（desugar-to-x-bind）", () => {
    test("混合段普通属性：href 拼接 + 依赖更新", async () => {
        const { root, engine } = mount(`<a href="/u/{{id}}">link</a>`, { id: 42 });
    ...
    test("混合段 class：classList diff，静态+动态合并，无字面 {{}} 泄漏", async () => {
        const { root, engine } = mount(`<div class="row {{type}}">x</div>`, { type: "primary" });
        const div = root.querySelector("div")!;
        expect(div.className).toBe("row primary");
        expect(div.getAttribute("class")).not.toContain("{{");
        engine.state.type = "secondary";
        await nextTick();
    ...
    test("整体单段 class（字符串）：透传原值走 normalizeClass", async () => {
        const { root, engine } = mount(`<div class="{{cls}}">x</div>`, { cls: "a b" });
    ...
    test('整体单段 boolean：disabled 透传原值（false 不禁用，避 string "false" 恒真坑）', async () => {
        const { root, engine } = mount(`<button disabled="{{locked}}">ok</button>`, {
            locked: true,
        });
    ...
    test("同属性显式 :attr 与插值冲突 → 编译期报错", () => {
        expect(() =>
            mount(`<div :class="x" class="row {{y}}">x</div>`, { x: "a", y: "b" }),
        ).toThrow(/插值冲突/);
    });
});

...
describe("x-text 文本绑定", () => {
    test("x-text 路径：初始渲染 + 状态变化更新 DOM", async () => {
        const { root, engine } = mount(`<span x-text="user.name"></span>`, { user: { name: "a" } });
  ...
  <span>b</span>
    ...
    test("x-text 表达式：多依赖自动收集 + 局部更新", async () => {
        const { root, engine } = mount(`<span x-text="user.first + ' ' + user.last"></span>`, {
            user: { first: "zhang", last: "san" },
        });
        expect(root).toEqualHTML(`<div>
  <span>zhang san</span>
</div>`);
        engine.state.user.last = "si";
        await nextTick();
        expect(root).toEqualHTML(`<div>
  <span>zhang si</span>
</div>`);
    ...
    test("数值被转为字符串写入 textContent", async () => {
        const { root, engine } = mount(`<span x-text="count"></span>`, { count: 7 });
        expect(root).toEqualHTML(`<div>
  <span>7</span>
</div>`);
...
describe("x-text 空值占位与 .hide 修饰符（ADR-0014）", () => {
    test("默认空值：null/undefined/NaN 渲染空串，0 渲染 '0'（NaN 归空为有意行为变更）", async () => {
        const { root, engine } = mount(`<span x-text="v"></span>`, { v: null });
        expect(root).toEqualHTML(`<div>
  <span></span>
</div>`);
        engine.state.v = undefined;
        await nextTick();
        expect(root).toEqualHTML(`<div>
  <span></span>
</div>`);
  ...
  <span>0</span>
    ...
    test("empty 占位：空值渲染指定文案，非空渲染原值，可随状态切换", async () => {
        const { root, engine } = mount(
            `<span x-text="v" x-text-options="{empty:'没有数据'}"></span>`,
            {
                v: null,
            },
        );
  ...
  <span>没有数据</span>
    ...
    test("emptyValues 附加到默认集（不覆盖）：[0] 让 0 也算空，默认 null 仍算空", async () => {
        ...
        const r1 = mount(`<span x-text="v" x-text-options="{empty:'无',emptyValues:[0]}"></span>`, {
            v: 0,
        });
  ...
  <span>无</span>
        ...
        const r2 = mount(`<span x-text="v" x-text-options="{empty:'无',emptyValues:[0]}"></span>`, {
            v: null,
        });
  ...
  <span>无</span>
        ...
        // 空附加 = 默认：0 不算空
        const r3 = mount(`<span x-text="v" x-text-options="{empty:'无',emptyValues:[]}"></span>`, {
            v: 0,
        });
  ...
  <span>0</span>
    ...
    test(".hide：空值隐藏宿主，恢复时还原原内联 display（flex）", async () => {
        const { root, engine } = mount(`<div style="display:flex" x-text.hide="v"></div>`, {
            v: null,
        });
    ...
    test(".hide 还原不固化计算值：无内联 display 时恢复为空串（让 CSS 类重新接管）", async () => {
        const { root, engine } = mount(`<div x-text.hide="v"></div>`, { v: null });
        const el = root.firstElementChild as HTMLElement;
    ...
    test(".hide 与 empty 并存：.hide 优先，文案不写入", async () => {
        const { root, engine } = mount(
            `<div x-text.hide="v" x-text-options="{empty:'无数据'}"></div>`,
            {
                v: null,
            },
        );

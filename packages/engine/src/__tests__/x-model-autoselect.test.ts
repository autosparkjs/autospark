...
describe("x-model autoSelect：单选", () => {
    test("默认开启：值不在集 → 自动选首项并回写", async () => {
        ...
        expect(engine.state.car).toBe("ZEEKR"); // 回写
    });
    ...
    test("default:true 项优先于首项", async () => {
        ...
        expect(engine.state.car).toBe("B");
    });
    ...
    test("default:true 多项 → 取第一个命中", async () => {
        ...
        await nextTick();
        expect(engine.state.car).toBe("B"); // 第一个 default:true
    });
    ...
    test("autoSelect:false → 旧行为（不勾中、不回写）", async () => {
        ...
        expect(engine.state.car).toBe("TESLA");
    });
    ...
    test("schema.autoSelect 声明（无模板声明时生效）", async () => {
        ...
        const engine = new AutoSpark(
            root,
            {
                car: configurable("TESLA", {
                    autoSelect: false,
                    choices: [{ value: "A", label: "甲" }] as any,
                }),
            },
            { storeOptions: { configManager: cm, configKey: "app" } as any },
        );
        await nextTick();
        expect(engine.state.car).toBe("TESLA"); // schema false 关闭
        ...
        const engine2 = new AutoSpark(
            ...
            { storeOptions: { configManager: cm2, configKey: "app" } as any },
        );
        await nextTick();
        expect(engine2.state.car).toBe("A"); // schema true 开启
    });
    ...
    test("模板声明优先于 schema（模板 false 覆盖 schema true）", async () => {
        ...
        await nextTick();
        expect(engine.state.car).toBe("TESLA"); // 模板 false 赢
    });
    ...
    test("空 choices → 静默不触发", async () => {
        ...
        expect(engine.state.car).toBe("TESLA"); // 无可选项不动
    });
    ...
    test("静态手写 option 的 select 同样参与", async () => {
        ...
        expect(engine.state.car).toBe("A");
    });
...
describe("x-model autoSelect：多选（过滤式）", () => {
    test("剔除数组中过期项并回写", async () => {
        ...
        await nextTick();
        const select = root.querySelector("select") as HTMLSelectElement;
        expect(engine.state.cars).toEqual(["A", "B"]); // 剔除过期项回写
    ...
    test("全部过期 → 不勾不回写（合法空态）", async () => {
        ...
        await nextTick();
        const select = root.querySelector("select") as HTMLSelectElement;
        expect(engine.state.cars).toEqual([]); // 过滤为空回写（剔除全部过期项）
    ...
    test("autoSelect:false → 过期项保留在 state（不过滤）", async () => {
        ...
        await nextTick();
        expect(engine.state.cars).toEqual(["A", "X"]); // 不回写
    });
});
...
describe("x-model autoSelect：三级级联全链", () => {
    test("切省 → 市自动选中回写 → 区县随之自动选中（链路闭合）", async () => {
        ...
        const engine = new AutoSpark(
            ...
            { storeOptions: { configManager: cm, configKey: "" } as any },
        );
        ...
        await nextTick();
        expect(engine.state.province).toBe("gd");
        expect(engine.state.city).toBe("gz"); // 自动选中回写（级联链闭合）
        expect(selects[1].value).toBe("gz");
        expect(engine.state.district).toBe("th"); // 二级自动跟进
        ...
        selects[0].dispatchEvent(new Event("change", { bubbles: true }));
        await nextTick();
        expect(engine.state.city).toBe("fz");
        expect(engine.state.district).toBe("gl_fz");
    });
    ...
    test("default:true 标记的市成为切换后的选中项", async () => {
        ...
        selects[0].dispatchEvent(new Event("change", { bubbles: true }));
        await nextTick();
        expect(engine.state.city).toBe("sz_gd"); // default:true 的深圳（非首项广州）
    });
});

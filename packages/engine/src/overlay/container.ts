...
/**
 * 覆盖层容器（ADR-0052 决策 13）：**每 engine 一个**，挂 `document.body` 下。
 *
 * - 首个覆盖层打开时**懒创建**；`engine.destroy()` 时经 {@link removeOverlayContainer} 整体移除。
 * - 容器本身是**透明壳**（无定位样式）——dialog 实例自带 fixed 遮罩，未来 popup/popover 的
 *   锚定定位不受容器布局约束；singleton 关闭的隐藏实例留存其中。
...
/** dialog 外壳类名契约（ADR-0052 决策 12；样式注入在 DialogDirective，结构在 OverlayInstance） */
export const MASK_CLASS = "autospark-dialog-mask";
export const PANEL_CLASS = "autospark-dialog";

/** 取（或懒建）本 engine 的覆盖层容器；SSR 返回 null。 */

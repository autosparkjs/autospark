...
选子节点式：实现优雅度差距实在（闭环 vs 全新协议），且根除断链类静默错误；「写在里面、渲染在外面」的间接性在引擎有既有先例（x-component 摘除、x-isolate 替换、x-if 自身 detach），非首创心智负担。
...
分支根本就是待编译子树的根：普通指令（`:class` / `x-text` / `x-on`…）随分支编译正常执行。同元素**结构指令**（ownsChildren 类：x-for / eager x-if / x-isolate）→ 编译期 warn + **跳过该分支**（「分支根循环 / 再条件化」语义混乱；keepalive x-if 不占子树，放行）。

...
- **状态**：Accepted（Round 3）｜⚠️ **部分废弃（2026-08-07）**：task 域（决策 3.2/3.5）未被采用——x-on async action 改用 `actions/<name>/*`、x-isolate 用 x-loading 覆盖层，task 域零消费者已移除。事件总线其余（engine/scope/directive/patch/render 域 + RuntimeObserverDispatcher + broadcast→emit）仍有效。见 glossary「task 域已废弃」。
- **日期**：2026-08-06

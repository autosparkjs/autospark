...
- `packages/engine`（包名 `autospark`）— **AutoTemplate Engine**：声明式模板渲染引擎，通过宿主元素上的 `x-*` / `@*` / `:*` 指令属性把 [AutoStore](https://github.com/zhangfisher/autostore) 响应式状态绑定到 DOM（Alpine.js 风格，最小声明 + 细粒度响应式 patch）。
- `docs/` — VitePress 中文文档站；`docs/demos/` 下是纯 HTML 可运行示例，依赖 engine 构建产物 `docs/public/template.js`（IIFE，全局变量 `AutoTemplateSpaces`）。

⚠️ `packages/engine` 与 `docs` 声明了 `workspace:*` 依赖（`autostore`、`@autostorejs/*` 等），这些包**不在本仓库**，来自关联的 autostore monorepo。engine 的构建配置 `noExternal: ["autostore", "really-relaxed-json"]` 会把 autostore 打包进产物；若依赖未正确链接，`bun test` / `bun run build` 会报 `Cannot find module`。
...
# engine 构建（tsup，esm/cjs/iife 三格式；成功后自动复制 IIFE 到 docs/public/template.js）
cd packages/engine && bun run build

# 文档站
cd docs && bun run dev                  # 本地开发
cd docs && bun run build                # 构建
...
- `src/engine.ts` — `AutoTemplateEngine` 门面类。构造入参 `(el, store | 裸状态, options)`：AutoStore 实例为借用（destroy 不销毁），裸状态为自建并拥有（ADR-0009）。生命周期 `compile → stop/start → destroy`。`engine.patch(selector, updater)` 支持运行时局部模板替换（ADR-0002），updater 返回值四态决定重建语义。
- `src/compile/compiler.ts` — 深度优先重建模板树：浅克隆元素、剥指令属性、建 scope、跑指令编译期生命周期；`{{}}` 文本/属性插值 desugar 为绑定（`compile/mustache.ts`，ADR-0004）。
- `src/scope.ts` — `AutoTemplateScope`：单元素上多指令的生命周期/订阅容器。`watch` 双轨：纯标识符路径走精准订阅，含运算符/函数调用的表达式走 with 求值（`isSimpleStatePath` 分流）。
...
- 基类 `AutoTemplateDirectiveBase` 的**静态**字段决定行为：`kind`（`Compile`/`Runtime`/`Hybrid`，ADR-0001）、`priority`（x-for=100 → x-if=80 → bind/on=50 → text/html=0）、`singleton`（同名去重）、`ownsChildren(info)`（结构指令接管子树编译，如 x-for / eager x-if / x-slot）。
...
- action 三入口（构造 `options.actions`、`engine.actions` Proxy 赋值、`<script type="actions">`）统一经 `utils/buildAction.ts` 包装，自动广播 `actions/<name>/*` 生命周期信号（x-loading 消费）。
...
- `packages/engine/docs/adr/` — ADR 0001~0029，源码注释大量以「ADR-XXXX 决策 N」形式回链。
...
- `packages/engine/CLAUDE.md` 内容已过期（引用不存在的 `.planning/` GSD 工作流），以本文件为准。

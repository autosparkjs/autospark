...
`x-for.virtual` 通过**虚拟列表**技术只渲染当前可见区域的项，滚动时动态替换内容，大幅提升大数据量列表的渲染性能。
...
| 选项 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `itemHeight` | `number` | 自动检测 | 每项的固定高度（像素）。无效值会退化为全量渲染 |
| `overscan` | `number` | `5` | 可见区域外额外渲染的项数 |
| `:data-index` | `string` | - | 滚动位置绑定的状态路径（双向绑定） |

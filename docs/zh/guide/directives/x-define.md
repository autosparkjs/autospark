...
`x-define` 是**声明性资源指令**——在模板中声明一个命名组件：把一段 DOM 连同它的**响应式状态（`state()`）、私有数据（`data`）、方法（`methods`）、生命周期钩子、作用域样式（`<style>`）**打包为可复用供体，经 `x-component:名称` 实例化消费。
...
### `<script setup>`：状态、数据、方法、钩子

`<script setup>`（布尔属性）或 `<script type="autospark/setup">` 声明组件逻辑，对象字面量按段合并：

```javascript
{
    state() { return { time: '...' } },   // 响应式状态：改了驱动更新
    data: { timer: null, cache: {} },     // 非响应式私有数据：改了不更新
...
| 段 | 形态 | 语义 |
| --- | --- | --- |
| `state()` | 工厂函数 | 响应式状态：模板可见、修改驱动更新；实例化时先注入，props 后覆盖 |
| `data` | 静态对象 | 非响应式私有数据：模板读不到，仅 method / 钩子内 `this.<键>` 访问 |
| `methods` | 对象 | 组件内部方法：`x-on` 调用、`this.其他方法()` 互调；查找止步组件边界 |
| 四阶段钩子 | 函数 | `created` / `mounted` / `beforeUnmount` / `unmounted` |

段名细节（读写规则、优先级、this 三件套）见[组件](../component.md)的「组件状态」与「组件数据」两节。
...
组件实例默认**封闭**——模板只能读自身 `state()` / `data`、props 与全局 state，外层 `x-data` 域不可见。声明侧经 `x-define-options` 开放：

<demo html="component/define-options.html"/>

开放后的继承基准由 `scope` 键决定（消费侧可经 `x-component-options.scope` 覆盖已开放组件的基准）：`'host'`（默认，消费处上下文）| `'declarer'`（声明处上下文，词法基准）。完整规则见[组件](../component.md)的「数据边界」一节。
...
| `scope` | `'host'` \| `'declarer'` | `'host'` | 开放状态的继承基准；声明 `scope` 而无 `open` 时 `warn` 忽略 |

```html
<!-- 两种开放写法等价 -->
<div x-define="card" x-define-options="{ open: true, scope: 'declarer' }">...</div>
...
- **完整教程**：组件来源、查找链、全局组件、`x-import` 远程加载见[组件](../component.md)。

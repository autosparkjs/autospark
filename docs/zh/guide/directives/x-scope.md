...
`x-scope` 是一个**零副作用的结构占位指令**，唯一作用是让一个「没有其他指令、没有 <span v-pre>`{{}}`</span> 插值」的纯容器元素在编译期建一个 scope——为后代 `x-define` 提供归属锚点，并在 scope 链上插入一道确定的边界。

```html
<div x-scope>
    <div x-define="loading">加载中…</div>
    <div x-loading="{ value:'on' }">内容</div>
</div>
```

`compileElement` 只在元素「含指令或插值」时才建 scope，否则只浅克隆。这导致一个仅作结构包裹的纯 `<div>` 不建 scope——其后代 `x-define` 无处就近归属（编译期 `warn` 丢弃），后代 scope 的 parent 链也会越过它落到更远的祖先。`x-scope` 就是填补这个缺口的最轻手段：`created` / `compile` / `destroy` 全是空操作，不建数据域、不注入 data、不订阅、不渲染。

## 快速入门

`x-scope` 让纯容器建 scope，使内部的 `x-define="loading"` 有归属锚点。该组件声明了骨架屏模板——`x-loading` 渲染时自动取用它替换内置旋转 loader。注意：`x-define` 元素本身**不会出现在页面上**。
...
1. **为后代 `x-define` 提供归属锚点**——`x-define` 收集时向上找最近 scope 挂 `components`，若无 `x-scope`（也无其他指令祖先），组件无处归属 → 编译期 `warn` 丢弃。`x-scope` 让任意 `<div>` 都能当组件容器。
...
- 元素本身**没有指令、没有插值**，但你需要在其内部声明 `x-define` → 加 `x-scope`。
- 想在 scope 链上**显式插入一道边界**，约束后代 scope 的 parent 继承范围 → 加 `x-scope`。
- 元素已有 `x-text` / `:class` / `x-data` 等任意指令或插值 → **不需要** `x-scope`，它已是 scope。

```html
<!-- ❌ 根 div 无指令无插值 → 不建 scope → x-define 被丢弃（warn） -->
<div>
    <div x-define="loading">加载中</div>
</div>

<!-- ✅ x-scope 让纯 div 建 scope → x-define 有归属锚点 -->
<div x-scope>
    <div x-define="loading">加载中</div>
</div>
```
...
| `x-scope` | ❌ | 为纯容器提供 scope 锚点（`x-define` 归属 + scope 链边界） |

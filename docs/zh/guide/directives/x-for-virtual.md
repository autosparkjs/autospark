...
```html
<div x-for.virtual="item of items" :key="item.id" x-for-options="{itemHeight:50}">
  <div class="list-item" x-text="item.name"></div>
...
`.virtual` 优化的是**渲染量**——只渲染可见区域的项。但上万项数据进入状态树时，AutoStore 缺省的**深层代理**仍会递归代理数组的每个成员及其嵌套对象，初始化耗时与内存开销随嵌套深度放大。渲染之外，状态侧同样要省——**最佳实践是对列表数组使用 `shallow` 包装**：仅做浅层代理，而不是做嵌套式的深层代理。

```javascript
import { AutoSpark, shallow } from "autospark";

const engine = new AutoSpark(el, {
  // 大列表仅浅层代理：数组一层 + 成员一层，孙级及以下读出即原始引用
  items: shallow(bigItems, 1),
});
```

- `shallow(list, 1)`：成员字段读写仍响应，项内 `:class` / `x-text` 等细粒度绑定照常工作；
- `shallow(list)`（缺省档）：最省——元素读出即原始引用，只靠整体增删 / 替换驱动列表。

两档的完整对比与适用边界（项内需要深层响应时不适用）见[x-for · 性能优化](./x-for.md#性能优化)。

大列表优化各管一层，配合使用互不冲突：

| 层面   | 手段       | 省掉什么                         |
| ------ | ---------- | -------------------------------- |
| 状态侧 | `shallow`  | 大列表深层代理的初始化耗时与内存 |
| 复用侧 | `:key`     | 结构变化时未变项的 DOM 重建      |
| 渲染侧 | `.virtual` | 可见区域之外全部项的 DOM 渲染    |

```html
<!-- shallow 包装状态 + :key 稳定复用 + .virtual 只渲染可见区 -->
<div
  class="list-container"
  x-for.virtual="item of items"
  :key="item.id"
...
:::warning 提示
大部份情况下，对大型对象数组进行深层代理是没有必要的，所以使用`shalow([....])`是虚拟列表的最佳实践。
:::
...
```html
<div x-for.virtual="item of items" :key="item.id" x-for-options="{itemHeight:50,overscan:5}">
  <!-- 可见区域外上下各多渲染 5 项 -->
</div>
```

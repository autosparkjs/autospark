...
在组件内用 `<script setup>` 声明数据与方法。`state()` 返回初始状态，`methods` 是组件方法：
...
- `state()` 返回的对象注入组件的**响应式状态域**，模板里直接用字段名（`count`）取用；
...
<!-- 传 props（对象形式）：count 覆盖 state() 默认值 -->
...
<div x-scope>
    ...
    <div x-component:counter data-count="10"></div>
    <!-- 传 props：count=100、step=5 覆盖 state() 默认 -->
    <div x-component:counter="{count: 100, step: 5 }"></div>
</div>
```
...
const engine = new AutoSpark(el, {}, {
    components: {
        ...
        card: `<div class="card"><span x-text="title"></span><script setup>{ state(){return{title:'默认'}} }</script></div>`,
    },
});
```

全局组件字符串入参首次使用时，按顶级节点数**自动包装**为「恰好一个带 `x-define` 的根元素」（详见下文「声明组件」），并懒预编译缓存。
...
`x-component` 在模板中实例化一个已声明的组件（ADR-0054 更名自 `x-use`）。语法职责分离：**属性参数承载组件名**、**值专职 props**。实例化时**宿主化身组件根**：宿主元素保留身份，组件快照子树编译挂入。
...
props 注入组件的**同一个 data 域**，合并顺序是 `state()` 默认先注入、props 后覆盖（外部优先）。三种形态：
...
- 更新 = 重求值后 `Object.assign` **只覆盖出现的键**——组件内部状态（用户交互改的）不被重置，绑定的状态对象删键后旧键残留在组件内（不做镜像同步）；
...
`x-component` 与结构指令（`x-if` / `x-for` / `x-slot` / `x-switch` / `x-tree` 等）**不能同元素**——编译期 `warn` 并跳过实例化。要控制组件显隐，把 `x-show` / `x-if` 写在外层包裹元素上：
...
组件的**响应式状态**由 `<script setup>` 的 `state()` 段声明——工厂函数返回初始状态对象，实例化时注入组件的响应式状态域：模板表达式直接用字段名取用，任何修改都驱动视图更新。
...
- **注入顺序**：`state()` 默认先注入，`x-component` props 后覆盖（同名键外部优先）；
- **模板可见**：`x-text="count"`、`{{ count }}` 直接读，改动即更新；
- **method / 钩子内**经 `this.data.count` 读写（`this.data` 是聚合视图，含 state + props + 全局 state）；
- **必须是函数**：每个实例独立调用求值，多实例不共享引用（写 `state: {...}` 静态对象会 `warn` 忽略）。

::: tip state 与全局 state
组件内 `this.state` 指向**全局 store 状态**（`engine.store.state`），组件自身响应式状态经 `this.data` 访问——两者是不同的引用。
:::
...
| `this.x` | 指向 | 可写 |
| --- | --- | --- |
| `this.data` | 组件聚合数据视图（`state()` 返回 + `x-component` props + 全局 state），**响应式** | ✅ 逐字段写触发更新 |
...
| `this.<data 键>` | 组件私有数据（见下文「组件数据」，非响应式） | ✅ |
...
组件实例除了响应式的 `state()`，还能有一份**非响应式**的私有数据——`<script setup>` 的 `data` 段（**静态对象**，非函数）。它专为「不该触发更新的实例内部数据」设计：定时器句柄、缓存、防抖标记、第三方库实例引用等。
...
| | `state()`（响应式状态） | `data`（私有数据） |
| --- | --- | --- |
| 形态 | 工厂函数，返回初始状态对象 | 静态对象字面量 |
| 响应式 | ✅ 改了触发更新 | ❌ 改了不更新 |
...
- **读**：`this.<键>`（如 `this.timer`）。优先级低于 method / `state` 字段 / 框架引用——同名时响应式字段优先、私有数据被遮蔽。
- **写**：`this.<键> = 值`——若该键已在 `state()` 返回值中出现则写响应式状态域，否则写私有数据。故 `this.timer = setInterval(...)` 自动落进 `data`。
- **跨生命周期共享**：`created` 设、`mounted` 用、`beforeUnmount` 清——这是 `data` 的核心用途（实例级、跨阶段、非响应式）。

::: warning data 不进模板
`data` 是组件私有的，**模板表达式读不到**——`<span x-text="timer">` 取不到（它是定时器句柄，本就不该显示）。要展示的数据放 `state()`，内部数据放 `data`。
:::

下面这个 demo 用 `data.timer` 跨 `mounted` / `beforeUnmount` 共享定时器句柄：勾选挂载时钟、取消勾选卸载（`beforeUnmount` 清理定时器）。
...
<div x-define="clock">
    ...
    <script setup>
        {
            state() { return { time: '00:00:00' } },
            data: { timer: null },          // 非响应式私有数据
            mounted() {
                this.timer = setInterval(() => {    // 句柄存 data（私有）
                    this.data.time = new Date().toLocaleTimeString();
                }, 1000);
            },
            beforeUnmount() {
                clearInterval(this.timer);          // 读 data（私有）清理
            },
        }
    </script>
</div>
...
<div x-define="timed">
    ...
    <script setup>
        {
            state() { return { tick: 0, timer: null } },
...
组件默认是**封闭**的：组件模板内只能读**自身 `state()`/`data`、`x-component` 传入的 props、全局 state**，读不到组件声明处外层的 x-data 域或局部变量。组件与外部的数据交互应经 **props 显式声明**——这是组件封装性的基础，也让组件在任何上下文中都可移植（行为不随消费位置变化）。
...
<div x-data="{ who: '声明处' }">
    <div x-scope>
        <div x-define="card" x-define-options="{ open: true, scope: 'declarer' }">
...
#### 基准的消费侧覆盖（`x-component-options`）

实例化时可用 `x-component-options="{ scope: 'host' | 'declarer' }"` 覆盖**已开放组件**的基准（消费处覆盖 > 作者声明 > 默认 `'host'`）。**封闭是作者契约**——消费侧不能打开封闭组件：对未 `open` 的组件声明 `x-component-options.scope` 会 `warn` 并保持封闭。

::: warning 三条失效告警（均 warn 一次 + 忽略）
- 作者声明了 `scope` 但没有 `open`——基准没有生效条件；
- 消费侧对封闭组件声明 `x-component-options.scope`；
...
子组件在 `state()` 里声明同名字段作默认值，props 覆盖之。详见上文「实例化 → props 注入」。
...
| `x-define.open` | 修饰符 | 开放数据边界（≡ `x-define-options="{open:true}"`，详见[数据边界](#数据边界默认封闭与-open)） |
| `x-define-options` | 元素属性 | 组件选项：`open`（开放边界开关）、`scope`（`'host'\|'declarer'` 基准，须配合 `open`） |
...
<div x-component:counter x-component-options="{ scope: 'declarer' }"></div>
```

除 `x-component-options.scope`（数据边界基准覆盖，见[数据边界](#数据边界默认封闭与-open)）外无其他指令选项与修饰符，值即组件名或 props 对象。
...
- **`x-component` 与结构指令互斥**：`x-component` 不能与 `x-if`/`x-for`/`x-slot`/`x-switch`/`x-tree` 同元素。要控制显隐，把结构指令写在外层包裹元素上。
...
`x-component` 与结构指令（`x-if`/`x-for`/`x-slot`/`x-switch`/`x-tree`）互斥，同元素会 `warn` 并跳过实例化。把结构指令挪到外层包裹元素：
...
`this.data` 是组件聚合数据视图（`state()` 返回 + `x-component` props），响应式可读写；`this.state` 是全局 store 状态；`this.scope` 是组件实例 scope。

### props 和 `state()` 的同名字段，哪个生效？

**props 生效**。合并顺序是 `state()` 默认先注入、`x-component` props 后覆盖（外部优先）。但 props 后续更新只覆盖它**声明的键**——组件内部改的字段（没被 props 声明的）不会被重置。
...
组件数据边界**默认封闭**——组件内只能读自身 `state()`/props 与全局 state，这是有意的封装设计（详见[数据边界](#数据边界默认封闭与-open)）。三个出路：

1. **props 显式传入**（推荐）：`<div x-component:card="{tip: tip }"></div>`；
2. 数据放**全局 state**（任意组件可见）；
3. 确需共享上下文的复杂组件，由组件作者声明 `x-define.open` 开放边界。

若控制台出现「未声明 open…x-component-options.scope 不生效」「基准仅在 open 声明时生效」「无声明处 scope」类 warn，都是边界配置失效提示，按[数据边界](#数据边界默认封闭与-open)一节修正声明即可。
